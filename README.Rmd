...
4. After the workshop, complete the lab exercises below. 
...
20. In a single pipeline, select `sched_arr_time`, `arr_delay`, `dest` from `flights`, left join `intl_airports` using `dest` as the 'key', and then count the airport names. Sort the output.
...
How are they different? How does the output change if we add `%>% ungroup()` to the end? Why might we want to add `ungroup()` after we've completed a grouping operation? 
